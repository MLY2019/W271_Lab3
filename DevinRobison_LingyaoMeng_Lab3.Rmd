...
ggplot(df.traffic) +
  ...
  geom_smooth(data=df.traffic.means, aes(x=as.factor(year), y=nghtfatrte_mean, group=2, color='green')) +
  geom_smooth(data=df.traffic.means, aes(x=as.factor(year), y=wkndfatrte_mean, group=2, color='red')) +
...
df.traffic.law.means <- df.traffic.restricted %>%
  ...
  summarise_at(vars(totfatrte,
                    bac08, perse, sbprim, sbsecon, sl70plus, gdl), list(mean=mean)) 
...
Total fatality rate is defined as the number of fatalities per 100,000 population.

Year based means were constructed as `df.traffic.means` in our initial EDA, and can be reused below.

Finally, to construct our simple linear regression, we'll extract all relevant columns as a new data frame `df.traffic.dummies`, and train our linear model. Note that passing a dataframe to `lm` produces a model where the first column is considered the dependant variable, and all subsquent columns are used as explanatory variables by the model.

```{r}
# Average totfatrate by year
df.traffic.means[c('year', 'totfatrte_mean')] %>%
  print(n=Inf)
